...
<head>
    ...
    <title>@yield('title', 'SE Shop - Next-Gen Tech & Electronics')</title>
            ...
            <div class="flex items-center justify-between h-20">
                ...
                <div class="flex items-center gap-8">
                    ...
                    <nav class="hidden md:flex items-center gap-6 text-sm font-semibold">
                        <a href="{{ route('home') }}" class="text-gray-300 hover:text-cyan-400 transition-colors {{ request()->routeIs('home') ? 'text-cyan-400 font-bold' : '' }}">Home</a>
                ...
                <div class="hidden lg:flex flex-1 max-w-md mx-8">
                    <form action="{{ route('shop.index') }}" method="GET" class="w-full relative">
                        <input type="text" name="q" value="{{ request('q') }}" placeholder="Search products, laptops, specs..." 
                ...
                <div class="flex items-center gap-3">
                    ...
                    @auth
                        @if(auth()->user()->isAdmin())
                            <a href="{{ route('admin.dashboard') }}" class="hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-indigo-950/80 hover:bg-indigo-900 text-xs font-bold text-indigo-300 border border-indigo-500/40 transition-all">
                                <i class="bi bi-speedometer2 text-cyan-400"></i> Admin Panel
                            </a>
                        @endif

                        <a href="{{ route('dashboard') }}" class="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-gray-900 hover:bg-gray-800 text-xs font-bold text-white border border-gray-800 transition-all">
                            <i class="bi bi-person-circle text-cyan-400"></i>
                            <span class="max-w-[100px] truncate">{{ auth()->user()->name }}</span>
                            <span class="px-1.5 py-0.5 rounded text-[9px] uppercase font-mono {{ auth()->user()->isAdmin() ? 'bg-indigo-900 text-indigo-300 border border-indigo-500/30' : 'bg-cyan-900/60 text-cyan-300 border border-cyan-500/30' }}">
                                {{ auth()->user()->role }}
                            </span>
                        </a>
                    ...
                    @else
                        <a href="{{ route('login') }}" class="px-3.5 py-1.5 text-xs font-bold rounded-xl bg-gray-900 border border-gray-700/80 text-cyan-400 hover:text-white hover:bg-gray-800 hover:border-cyan-500/50 shadow-md transition-all flex items-center gap-1.5">
                            <i class="bi bi-box-arrow-in-right"></i> Sign In
                        </a>
                        <a href="{{ route('register') }}" class="hidden sm:inline-flex px-3.5 py-1.5 text-xs font-bold rounded-xl bg-cyan-500 hover:bg-cyan-400 text-gray-950 shadow-md shadow-cyan-500/20 transition-all">
                            Register
                        </a>
                    @endauth
                    ...
                    <button type="button" id="cart-drawer-toggle" class="relative p-2.5 rounded-xl bg-gray-900 border border-gray-800 text-gray-300 hover:text-white hover:border-cyan-500/50 transition-all ml-1">
                        ...
                        @endphp
                        <span id="cart-count-badge" class="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-cyan-500 text-gray-950 font-extrabold text-xs flex items-center justify-center shadow-lg shadow-cyan-500/50 {{ $cartCount > 0 ? '' : 'hidden' }}">
                            {{ $cartCount }}
                        </span>
                    </button>
                </div>
            ...
            <div class="w-screen max-w-md bg-[#111827] border-l border-gray-800 text-gray-100 flex flex-col shadow-2xl">
                ...
                <div class="p-6 border-t border-gray-800 bg-gray-900/80 space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        <a href="{{ route('cart.index') }}" class="w-full py-3 text-center text-sm font-semibold rounded-xl bg-gray-800 hover:bg-gray-700 text-gray-200 border border-gray-700 transition-colors">
                            View Full Cart
                        </a>
                        <a href="{{ route('checkout.index') }}" class="w-full py-3 text-center text-sm font-bold rounded-xl cyber-glow-btn">
    ...
    <footer class="mt-24 bg-[#080b12] border-t border-gray-800/80 text-gray-400 text-sm">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-12">
                ...
                <div>
                    ...
                    <ul class="space-y-2 text-xs">
                        <li><a href="{{ route('shop.index') }}" class="hover:text-cyan-400 transition-colors">All Products</a></li>
                        <li><a href="{{ route('shop.index', ['category' => 'laptops-workstations']) }}" class="hover:text-cyan-400 transition-colors">Dev Laptops</a></li>
                        <li><a href="{{ route('shop.index', ['category' => 'smart-peripherals']) }}" class="hover:text-cyan-400 transition-colors">Mechanical Keyboards</a></li>
                        <li><a href="{{ route('shop.index', ['category' => 'audio-studio-tech']) }}" class="hover:text-cyan-400 transition-colors">ANC Headphones</a></li>
                    </ul>
                </div>
            ...
            <div class="mt-12 pt-8 border-t border-gray-800/60 flex flex-col md:flex-row items-center justify-between text-xs text-gray-500">
                <p>&copy; {{ date('Y') }} SE Shop Inc. All rights reserved.</p>
                <div class="flex items-center gap-6 mt-4 md:mt-0">
                    <a href="{{ route('privacy') }}" class="hover:text-cyan-400 transition-colors">Privacy Policy</a>
                    <a href="{{ route('terms') }}" class="hover:text-cyan-400 transition-colors">Terms of Service</a>
                    <a href="{{ route('security') }}" class="hover:text-cyan-400 transition-colors">Security</a>
                </div>
                    ...
                    if (!data.cart || data.cart.length === 0) {
                        drawerItemsList.innerHTML = `
                            <div class="text-center py-12 space-y-3">
                                ...
                                <a href="{{ route('shop.index') }}" onclick="closeDrawer()" class="inline-block px-4 py-2 text-xs font-bold rounded-lg bg-blue-600 hover:bg-blue-500 text-white transition-colors">Browse Products</a>
                            </div>
                        `;
                        return;
                    }
