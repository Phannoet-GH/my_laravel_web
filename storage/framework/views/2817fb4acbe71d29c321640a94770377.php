<?php $__env->startSection('title', 'Admin Dashboard - SE Shop'); ?>
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        ...
        <div class="stat-metric-card space-y-1">
            ...
            <div class="text-2xl font-black text-white">$<?php echo e(number_format($totalSales, 2)); ?></div>
        ...
        <div class="stat-metric-card space-y-1">
            ...
            <div class="text-2xl font-black text-cyan-400"><?php echo e($ordersCount); ?></div>
        ...
        <div class="stat-metric-card space-y-1">
            ...
            <div class="text-2xl font-black text-purple-400"><?php echo e($totalUsers); ?></div>
        ...
        <div class="stat-metric-card space-y-1">
            ...
            <div class="text-2xl font-black text-amber-400"><?php echo e($pendingOrders); ?></div>
    ...
    <?php if(isset($lowStockProducts) && $lowStockProducts->count() > 0): ?>
        ...
        <div class="p-4 rounded-2xl bg-amber-950/80 border border-amber-500/40 text-amber-300 text-xs flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h4 class="font-extrabold text-white text-sm">Low Stock Inventory Warning (<?php echo e($lowStockProducts->count()); ?> SKUs)</h4>
                    <p class="text-amber-200/80 text-[11px]">The following SKUs have 5 or fewer items remaining in inventory: 
                        <?php $__currentLoopData = $lowStockProducts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $lowProd): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <span class="font-bold text-white bg-amber-900/60 px-2 py-0.5 rounded border border-amber-500/30 ml-1"><?php echo e($lowProd->name); ?> (<?php echo e($lowProd->stock); ?> left)</span>
    ...
    <div class="glass-panel border-gray-800 overflow-hidden">
        <div class="p-4 border-b border-gray-800 bg-gray-900/60 flex items-center justify-between">
            ...
            <span class="text-xs text-gray-400"><?php echo e(count($allOrders)); ?> Customer Orders Platform-wide</span>
        </div>
                    ...
                    <?php $__empty_1 = true; $__currentLoopData = $allOrders; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $order): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                        <tr class="hover:bg-gray-900/40">
                            <td class="p-4 font-mono font-bold text-cyan-400">
                                <a href="<?php echo e(route('orders.show', $order->order_number)); ?>" target="_blank" class="hover:underline flex items-center gap-1">
                                    <?php echo e($order->order_number); ?>

                                    <i class="bi bi-box-arrow-up-right text-[10px]"></i>
                                </a>
                                <span class="text-[10px] text-gray-500 font-sans block mt-0.5"><?php echo e($order->created_at->format('M d, Y')); ?></span>
                            </td>
                            <td class="p-4">
                                <span class="font-bold text-white block"><?php echo e($order->customer_name); ?></span>
                                <span class="text-[11px] text-gray-400 block"><?php echo e($order->customer_email); ?></span>
                                <span class="text-[10px] text-gray-500 font-mono"><?php echo e($order->customer_phone); ?></span>
                                <?php if($order->user): ?>
                                    <span class="inline-block mt-1 px-2 py-0.5 rounded bg-purple-950 text-purple-300 text-[9px] font-bold border border-purple-500/30">
                                        User ID: #<?php echo e($order->user->id); ?>
                                ...
                                <div class="space-y-1 max-w-xs">
                                    <?php $__currentLoopData = $order->items; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <div class="text-[11px] text-gray-300 flex items-center justify-between gap-2 bg-gray-950/40 px-2 py-1 rounded border border-gray-800/60">
                                            <span class="truncate font-medium"><?php echo e($item->product_name); ?></span>
                                            <span class="font-bold text-cyan-400 whitespace-nowrap">&times;<?php echo e($item->quantity); ?></span>
                                        </div>
                                    <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                                </div>
                            </td>
                            <td class="p-4 font-bold text-white">
                                ...
                                <span class="text-[10px] text-gray-500 block uppercase font-mono"><?php echo e($order->payment_method); ?></span>
                            </td>
                            <td class="p-4">
                                <?php
                                    $statusClasses = [
                                ...
                                ?>
                                <span class="px-2.5 py-1 rounded-full text-[10px] font-bold uppercase border <?php echo e($statusClasses[$order->status] ?? 'bg-gray-800 text-gray-300'); ?>">
                                    <?php echo e($order->status); ?>

                                </span>
                            </td>
                            <td class="p-4 text-right">
                                <form action="<?php echo e(route('admin.orders.status', $order->id)); ?>" method="POST" class="inline-flex items-center gap-1.5">
                                    <?php echo csrf_field(); ?>
        ...
        <div class="p-5 border-b border-gray-800 bg-gray-900/80 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <div class="flex items-center gap-3">
                <span class="text-xs text-gray-400 font-bold bg-gray-950 px-3 py-1 rounded-lg border border-gray-800">
                    <i class="bi bi-box-seam text-cyan-400 mr-1"></i> <?php echo e(count($products)); ?> Active SKUs
                </span>
                    ...
                    <?php $__currentLoopData = $products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $prod): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        <tr class="hover:bg-gray-900/50 transition-colors">
                            <td class="p-4">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-xl bg-gray-950 border border-gray-800 overflow-hidden flex-shrink-0">
                                        <img src="<?php echo e($prod->image); ?>" class="w-full h-full object-cover">
                                    </div>
                                    <div class="min-w-0">
                                        <a href="<?php echo e(route('shop.show', $prod->slug)); ?>" target="_blank" class="font-bold text-white hover:text-cyan-300 block truncate transition-colors">
                                            <?php echo e($prod->name); ?>

                                        </a>
                                        <span class="text-[10px] text-gray-500 font-mono block"><?php echo e($prod->slug); ?></span>
                                    </div>
                                </div>
                            </td>
                            <td class="p-4 text-cyan-400 font-semibold"><?php echo e($prod->category->name ?? 'Uncategorized'); ?></td>
                            <td class="p-4 font-bold text-white">
                                $<?php echo e(number_format($prod->active_price, 2)); ?>

                                <?php if($prod->sale_price): ?>
                                    <span class="text-[10px] text-emerald-400 line-through block font-normal">$<?php echo e(number_format($prod->price, 2)); ?></span>
                                <?php endif; ?>
                            </td>
                            <td class="p-4">
                                <?php if($prod->stock > 10): ?>
                                    <span class="font-bold px-2.5 py-1 rounded-full text-[10px] uppercase inline-flex items-center gap-1 bg-emerald-950/80 text-emerald-400 border border-emerald-500/30">
                                        <i class="bi bi-check-circle-fill text-[9px]"></i> <?php echo e($prod->stock); ?> Units
                                    </span>
                                <?php elseif($prod->stock > 0): ?>
                                    <span class="font-bold px-2.5 py-1 rounded-full text-[10px] uppercase inline-flex items-center gap-1 bg-amber-950/80 text-amber-400 border border-amber-500/30 animate-pulse">
                                        <i class="bi bi-exclamation-triangle-fill text-[9px]"></i> <?php echo e($prod->stock); ?> Low Stock
                                    </span>
                                <?php else: ?>
                            ...
                            <td class="p-4 text-right space-x-2">
                                <button type="button" onclick="editProduct(<?php echo e(json_encode($prod)); ?>)" class="px-2.5 py-1 bg-indigo-950 hover:bg-indigo-900 text-indigo-300 rounded-lg text-[11px] font-bold border border-indigo-500/30 transition-colors inline-flex items-center gap-1">
                                    <i class="bi bi-pencil-square"></i> Edit
                                </button>
                                <form action="<?php echo e(route('admin.products.destroy', $prod->id)); ?>" method="POST" class="inline" onsubmit="return confirm('Delete <?php echo e($prod->name); ?>?')">
                                    <?php echo csrf_field(); ?>
...
<div id="add-product-modal" class="fixed inset-0 bg-black/80 backdrop-blur-sm z-50 flex items-center justify-center p-4 hidden">
    <div class="glass-panel p-6 border-gray-800 w-full max-w-2xl max-h-[90vh] overflow-y-auto space-y-4">
        ...
        <form action="<?php echo e(route('admin.products.store')); ?>" method="POST" class="space-y-4 text-xs">
            <?php echo csrf_field(); ?>
            <div>
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\YEAR IV\SEMESTER I\WB-III\project\my-laravel-app\resources\views/admin/dashboard.blade.php ENDPATH**/ ?>