<?php $__env->startSection('title', 'SE Shop - Next-Gen Tech & Developer Hardware'); ?>
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            <div class="lg:col-span-7 space-y-6 text-center lg:text-left">
                ...
                <div class="flex flex-col sm:flex-row items-center justify-center lg:justify-start gap-4 pt-2">
                    <a href="<?php echo e(route('shop.index')); ?>" class="w-full sm:w-auto px-8 py-4 rounded-xl cyber-glow-btn text-base font-bold flex items-center justify-center gap-2">
                        Explore Catalog <i class="bi bi-arrow-right"></i>
                    </a>
                    <a href="<?php echo e(route('shop.index', ['category' => 'laptops-workstations'])); ?>" class="w-full sm:w-auto px-8 py-4 rounded-xl bg-gray-900/90 hover:bg-gray-800 text-gray-200 border border-gray-700/80 text-base font-semibold transition-all flex items-center justify-center gap-2">
            ...
            <div class="lg:col-span-5 relative">
                <div class="glass-panel p-6 border-gray-800 shadow-2xl relative overflow-hidden group">
                    ...
                    <?php if($featuredProducts->first()): ?>
                        <?php $heroProduct = $featuredProducts->first(); ?>
                        <img src="<?php echo e($heroProduct->image); ?>" alt="<?php echo e($heroProduct->name); ?>" class="w-full h-72 object-cover rounded-xl bg-gray-900 transform group-hover:scale-105 transition-transform duration-500">
                        
                        <div class="mt-6 space-y-2">
                            <div class="flex items-center justify-between">
                                <span class="text-xs text-cyan-400 font-bold uppercase tracking-wider"><?php echo e($heroProduct->category->name ?? 'Hardware'); ?></span>
                                <div class="flex items-center gap-1 text-amber-400 text-xs">
                                    <i class="bi bi-star-fill"></i>
                                    <span class="font-bold text-gray-200"><?php echo e($heroProduct->rating); ?></span>
                                </div>
                            </div>
                            <h3 class="text-xl font-bold text-white"><?php echo e($heroProduct->name); ?></h3>
                            <p class="text-xs text-gray-400 line-clamp-2"><?php echo e($heroProduct->tagline); ?></p>

                            <div class="flex items-center justify-between pt-4 border-t border-gray-800">
                                <div>
                                    <span class="text-xs text-gray-500 block line-through">$<?php echo e(number_format($heroProduct->price, 2)); ?></span>
                                    <span class="text-2xl font-extrabold text-white">$<?php echo e(number_format($heroProduct->active_price, 2)); ?></span>
                                </div>
                                
                                <?php if(auth()->check() && auth()->user()->isAdmin()): ?>
                                    <a href="<?php echo e(route('admin.dashboard')); ?>" class="px-4 py-2 rounded-xl bg-indigo-950 hover:bg-indigo-900 border border-indigo-500/40 text-indigo-300 text-xs font-bold flex items-center gap-1.5 transition-colors">
                                        <i class="bi bi-speedometer2"></i> Admin Panel
                                    </a>
                                <?php else: ?>
                                    <form action="<?php echo e(route('cart.add')); ?>" method="POST">
                                        <?php echo csrf_field(); ?>
                                        <input type="hidden" name="product_id" value="<?php echo e($heroProduct->id); ?>">
...
<section class="py-20">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex flex-col md:flex-row md:items-end justify-between mb-12">
            ...
            <a href="<?php echo e(route('shop.index')); ?>" class="text-sm font-semibold text-cyan-400 hover:text-cyan-300 mt-4 md:mt-0 flex items-center gap-1">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6">
            <?php $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <a href="<?php echo e(route('shop.index', ['category' => $category->slug])); ?>" class="glass-panel p-6 glass-panel-hover group flex flex-col justify-between h-48 border-gray-800/80">
                    <div class="flex items-center justify-between">
                        <div class="w-10 h-10 rounded-xl bg-blue-950/80 border border-blue-500/30 flex items-center justify-center text-cyan-400 text-xl group-hover:scale-110 transition-transform">
                            <i class="bi bi-<?php echo e($category->icon); ?>"></i>
                        </div>
                        <span class="text-[10px] font-extrabold px-2 py-0.5 rounded bg-gray-800 text-gray-300 uppercase"><?php echo e($category->badge ?? 'GEAR'); ?></span>
                    </div>

                    <div>
                        <h3 class="text-base font-bold text-white group-hover:text-cyan-400 transition-colors"><?php echo e($category->name); ?></h3>
                        <p class="text-xs text-gray-400 mt-1"><?php echo e($category->products_count); ?> Products Available</p>
                    </div>
                </a>
            ...
            <?php $__currentLoopData = $trendingProducts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $product): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="glass-panel p-5 glass-panel-hover flex flex-col justify-between border-gray-800 relative group">
                    ...
                    <div>
                        ...
                        <span class="text-[11px] font-bold text-cyan-400 uppercase tracking-wider"><?php echo e($product->category->name ?? 'Gear'); ?></span>
                        ...
                        <p class="text-xs text-gray-400 line-clamp-2 mt-1"><?php echo e($product->tagline); ?></p>
                    </div>
                    ...
                    <div class="mt-6 pt-4 border-t border-gray-800 flex items-center justify-between">
                        ...
                        <?php if(auth()->check() && auth()->user()->isAdmin()): ?>
                            <a href="<?php echo e(route('admin.dashboard')); ?>" class="w-10 h-10 rounded-xl bg-indigo-950 hover:bg-indigo-900 border border-indigo-500/30 text-indigo-300 flex items-center justify-center transition-colors" title="Manage in Admin Dashboard">
...
<?php echo $__env->make('layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH D:\YEAR IV\SEMESTER I\WB-III\project\my-laravel-app\resources\views/home.blade.php ENDPATH**/ ?>